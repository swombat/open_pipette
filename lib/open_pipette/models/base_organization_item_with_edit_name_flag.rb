...
module OpenPipette
  class BaseOrganizationItemWithEditNameFlag
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenPipette::BaseOrganizationItemWithEditNameFlag` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenPipette::BaseOrganizationItemWithEditNameFlag`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'people_count')
        self.people_count = attributes[:'people_count']
      end
      ...
      if attributes.key?(:'edit_name')
        self.edit_name = attributes[:'edit_name']
      end
    end
    ...
    def hash
      [id, company_id, owner_id, name, active_flag, picture_id, country_code, first_char, add_time, update_time, visible_to, label, owner_name, cc_email, email_messages_count, people_count, activities_count, done_activities_count, undone_activities_count, files_count, notes_count, followers_count, address, address_subpremise, address_street_number, address_route, address_sublocality, address_locality, address_admin_area_level_1, address_admin_area_level_2, address_country, address_postal_code, address_formatted_address, open_deals_count, related_open_deals_count, closed_deals_count, related_closed_deals_count, won_deals_count, related_won_deals_count, lost_deals_count, related_lost_deals_count, next_activity_date, next_activity_time, next_activity_id, last_activity_id, last_activity_date, edit_name].hash
    end
