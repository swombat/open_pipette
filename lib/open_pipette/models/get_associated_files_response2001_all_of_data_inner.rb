...
module OpenPipette
  # The file data
  class GetAssociatedFilesResponse2001AllOfDataInner
    # The ID of the file
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenPipette::GetAssociatedFilesResponse2001AllOfDataInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenPipette::GetAssociatedFilesResponse2001AllOfDataInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, product_id, add_time, update_time, file_name, file_size, active_flag, inline_flag, remote_location, remote_id, s3_bucket, product_name, url, name, description].hash
    end
