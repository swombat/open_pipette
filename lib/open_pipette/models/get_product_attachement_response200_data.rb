...
module OpenPipette
  ...
  class GetProductAttachementResponse200Data
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenPipette::GetProductAttachementResponse200Data` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenPipette::GetProductAttachementResponse200Data`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [product_id, item_price, quantity, discount, discount_type, product_variation_id, comments, tax, tax_method, enabled_flag, billing_frequency, billing_frequency_cycles, billing_start_date, id, company_id, deal_id, sum, currency, add_time, last_edit, active_flag, name].hash
    end
