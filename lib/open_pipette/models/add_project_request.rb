  ...
  class AddProjectRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenPipette::AddProjectRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenPipette::AddProjectRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @phase_id.nil?
        invalid_properties.push('invalid value for "phase_id", phase_id cannot be nil.')
      end
    ...
    def hash
      [title, board_id, phase_id, description, status, owner_id, start_date, end_date, deal_ids, org_id, person_id, labels, template_id].hash
    end
